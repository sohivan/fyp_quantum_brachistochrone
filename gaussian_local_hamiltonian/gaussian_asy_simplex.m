function [best_time,best_param] = gaussian_asy_simplex(param,ent,gz,rn,H1,H2,H12,dt,tol,filename)
	...
	for i = 1:vertices
		coef_i = param(i,:);
		[timeholder(i),s(i)] = gaussian_asy_evolver(coef_i,ent,gz,...
            H1,H2,H12,dt);
	end
	...
	for k = 1:1000
		...
		[time_refl,s_refl] = gaussian_asy_evolver(cref,ent,gz,...
            H1,H2,H12,dt);
		...
		elseif time_refl < time_best
			   ...
			   [time_exp,s_exp] = gaussian_asy_evolver(cexp,ent,gz,H1,H2,...
                   H12,dt);
		...
		elseif timeholder(end-1) <= time_refl && time_refl < time_worst
				...
				[time_con,s_con] = gaussian_asy_evolver(ccon,ent,gz,H1,H2,...
                    H12,dt);
				% contract better
		...
		elseif time_worst <= time_refl
				...
				% shrink
					...
					for i = 1:vertices
						coef_i = param(i,:);
						[timeholder(i), s(i)] = gaussian_asy_evolver(coef_i,...
                            ent,gz,H1,H2,H12,dt);
					end                  
				end       
		end
		...
		if strd < tol
            ...
            amplitude_rand = randi([-rn,rn],1,2);
            mean_rand = rand(1,2)*2;
            sd_rand = rand(1,2)*rn;
			param_rand = [th_rand, amplitude_rand, mean_rand, sd_rand];
			[time_rand, s_rand] = gaussian_asy_evolver(param_rand,...
                ent,gz,H1,H2,H12,dt);

			if time_rand > timeholder(1)
			% if the random point doesn't result in a better timing, 
%             we terminate the search
				break
			else
			% if the random point does result in a better timing, 
%             we will use that new point as the best new point

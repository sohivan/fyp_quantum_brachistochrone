function [best_time,best_param] = fourier_simplex(param,ent,w,gz,rn,H12,HXX,dt,tol,terms,filename)
	...
	for i = 1:vertices
		coef_i = param(i,:);
		[timeholder(i),s(i)] = fourier_evolver(coef_i,ent,gz,w,H12,HXX,dt,terms,filename);
	end
	...
	for k = 1:1000
		...
		[time_refl,s_refl] = fourier_evolver(cref,ent,gz,w,H12,HXX,dt,terms,filename);
		...
		elseif time_refl < time_best
			   ...
			   [time_exp,s_exp] = fourier_evolver(cexp,ent,gz,w,H12,HXX,dt,terms,filename);
		...
		elseif timeholder(end-1) <= time_refl && time_refl < time_worst
				...
				[time_con,s_con] = fourier_evolver(ccon,ent,gz,w,H12,HXX,dt,terms,filename);
				% contract better
		...
		if strd < tol
			...
			ps_rand = zeros(1,2);
			coef_rand = randi([-rn,rn],1,2*terms);
			param_rand = [th_rand, ps_rand, coef_rand];
			param_rand(6) = 0;
			[time_rand, s_rand] = fourier_evolver(param_rand,ent,gz,w,H12,HXX,dt,terms,filename);

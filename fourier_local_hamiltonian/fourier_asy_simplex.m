function [best_time,best_param] = fourier_asy_simplex(param,ent,w,gz,rn,H12,H1,H2,dt,tol,terms,filename)
	...
	for i = 1:vertices
		coef_i = param(i,:);
		[timeholder(i),s(i)] = fourier_asy_evolver(coef_i,ent,gz,w,H12,H1,H2,dt,terms);
	end
	...
	for k = 1:1000
		...
		[time_refl,s_refl] = fourier_asy_evolver(cref,ent,gz,w,H12,H1,H2,dt,terms);
		...
		elseif time_refl < time_best
			   ...
			   [time_exp,s_exp] = fourier_asy_evolver(cexp,ent,gz,w,H12,H1,H2,dt,terms);
		...
		elseif timeholder(end-1) <= time_refl && time_refl < time_worst
				...
				[time_con,s_con] = fourier_asy_evolver(ccon,ent,gz,w,H12,H1,H2,dt,terms);
				% contract better
		...
		if strd < tol
			...
			coef_rand = randi([-rn,rn],1,4*terms);
			param_rand = [th_rand, coef_rand];
			param_rand(5:6) = 0;
			[time_rand, s_rand] = fourier_asy_evolver(param_rand,ent,gz,w,H12,H1,H2,dt,terms);

function [best_time,best_param] = simplex_non_local(param,ent,dt,tol,filename)
	...
	for i = 1:vertices
		coef_i = param(i,:);
		[timeholder(i),s(i)] = entropy(coef_i,ent,dt);
	end
    ...
    for k = 1:5000
		...
		[time_refl, s_refl] = entropy(cref,ent,dt);
		...
		elseif time_refl < time_best
			   ...
			   [time_exp, s_exp] = entropy(cexp,ent,dt);
		...
		elseif timeholder(end-1) <= time_refl && time_refl < time_worst
				...
				[time_con, s_con] = entropy(ccon,ent,dt);
				% contract better
        ...
        if strd < tol
			...
			ps_rand = zeros(1,2);
			param_rand = [th_rand, ps_rand];
			[time_rand, s_rand] = entropy(param_rand,ent,dt);
...
% 	store = [best_s, best_time, ent, tol, dt, best_param, mod(best_param(1:4)/pi,2)];
